...
// INDEX.JS FOR GIOVANNA WORKING (MEDIA)
/*
import express from 'express';
import bodyParser from 'body-parser';
import mongoose from 'mongoose';
...
app.use('/api/users', userRoutes);
app.use('/api/channels', channelRoutes);
//app.use('/api/public/media', express.static(__dirname + '/public/media'));
app.use('/api/public/media', express.static(path.join(__dirname, '/public/media')));
...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        const extension = file.originalname.split(".").pop();
        const filename = `${Date.now()}_${req.params.id}.${extension}`;
...
try {
    app.post('/api/:id/uploadMedia', upload.single('file'), (req, res) => {
        //console.log("REQ: ", req);
        //console.log("RES: ", res);
        res.status(200).json(req.file);
    });
